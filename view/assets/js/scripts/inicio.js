          ...
          if(!isEmpty(msg.dados)){
            msg.dados.forEach(element => {
              ...
              if(!isEmpty(entrada) && !isEmpty(saida)){
                card_body.innerHTML += `<ul class="list-group" data-conteudo="horarios-horarios-hoje"><li class="list-group-item d-flex align-items-center justify-content-between list-group-title"><b>Horário</b>&nbsp;<div><button type="button" class="btn btn-primary" id="registrar-entrada" onclick="registrarEntrada(event)">Registrar entrada</button>&nbsp;<button type="button" class="btn btn-secondary" id="registrar-saida" onclick="registrarSaida(event)">Registrar saída</button></div></li><li class="list-group-item d-flex align-items-center justify-content-between"><div><span class="text-muted">Entrada:</span><span id="hr-entrada">&nbsp;${`-`}</span></div><span class="text-muted contador-atraso">Sem atraso</span></li><li class="list-group-item d-flex align-items-center justify-content-between"><div><span class="text-muted">Saída:</span><span id="hr-saida">&nbsp;${`-`}</span></div><span class="text-muted contador-atraso">Sem atraso</span></li><li class="list-group-item contador-periodos"><div><span class="text-muted">Tempo decorrido:</span><span data-conteudo="tempo-decorrido-horario">&nbsp;${"-"/* cronometrar(entrada, moment(), 'crescente') */}</span></div><div><span class="text-muted">Tempo restante:</span><span data-conteudo="tempo-restante-horario">&nbsp;${"-"/*cronometrar(entrada, moment(), 'decrescente').split('').includes('-') ? '0:00:00' : cronometrar(entrada, moment(), 'decrescente')*/}</span></div></li></ul><br>`;
      ...
      if(msg.mensagem.toLowerCase() === 'dados recebidos'){
        if(!isEmpty(msg.dados)){
          ...
          msg.dados.forEach(element => {
            ...
            // Se horário tiver sido definido, botão é bloqueado
            setTimeout(() => {
              try {
                entrada ? document.querySelector('#registrar-entrada').setAttribute('disabled', 'true') : '';
                saida ? document.querySelector('#registrar-saida').setAttribute('disabled', 'true') : '';
